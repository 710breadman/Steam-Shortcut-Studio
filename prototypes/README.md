...
## Apply Changes (shortcuts) is now live
Per `docs/UI_UX_TARGET.md`'s "No fake safety" principle, Apply Changes only does what it can back up and verify. It now performs **real, transactional `shortcuts.vdf` writes**:
- Scope is the selected rows, or all rows if nothing is selected (same convention as Preview).
- Each row is converted via `ui_library_adapter.writable_game_from_library_row`, which only resolves eligible non-Steam rows whose launch target exists on disk right now — native Steam rows, empty launch targets, and missing executables are always skipped and listed by reason in Preview/Apply, never silently dropped.
- The write itself runs on a background job (same `BackgroundJobQueue`/`_poll_jobs` pattern as scans) through the unmodified `shortcut_transactions.upsert_games_transactional` — the same backup → write → verify → rollback service the legacy UI uses. If Steam is running it is closed before the write and reopened after, matching legacy `ui.py` behavior; any verification failure auto-rolls back and is recorded on the Backups screen, which already shows it via `list_transaction_history()`.
- **Artwork writes to Steam remain gated**: Apply Changes does not touch artwork at all — see below for what artwork actions *are* live now.
